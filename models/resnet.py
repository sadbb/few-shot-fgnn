...
class ResNet(nn.Module):
    def __init__(self, layers=[3, 3, 3]):
...
class SELayer(nn.Module):
    def __init__(self):
        ...
        self.c1 = nn.Conv2d(1, 1, kernel_size=[5,1])

    def forward(self,x):
        ns, c, d, _ = x.size()
        x = torch.stack([x_.sum(dim=0) for x_ in x.chunk(5, dim=0)], dim=0)
        x = F.avg_pool2d(x, d).squeeze()
        x = self.c1(x.unsqueeze(0).unsqueeze(0)).squeeze()
        x = 2 * torch.sigmoid(F.normalize(x, dim=0)) 
        return x.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).repeat(ns, 1, d, d) 
